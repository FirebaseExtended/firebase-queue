...
function QueueWorker(queueRef, processId, processingFunction) {
  ...
  if (!_.isFunction(processingFunction)) {
    var error = 'No processing function provided';
 ...
 */
QueueWorker.prototype._resetItem = function(itemRef) {
  var self = this,
      retries = 0;
  ...
  itemRef.transaction(function(queueItem) {
    ...
    if (error) {
      if (++retries < MAX_TRANSACTION_ATTEMPTS) {
        ...
        setImmediate(self._resetItem.bind(self), itemRef);
      } else {
        logger.error(self._getLogEntry('reset item errored too many times, ' +
 ...
 *   deleted.
 */
QueueWorker.prototype._resolve = function(newQueueItem) {
  var self = this,
      retries = 0;
    ...
    self.currentItemRef.transaction(function(queueItem) {
      ...
      if (error) {
        if (++retries < MAX_TRANSACTION_ATTEMPTS) {
          ...
          setImmediate(self._resolve.bind(self), newQueueItem);
        } else {
          logger.error(self._getLogEntry('resolve item errored too many ' +
            'times, no longer retrying'), error);
        }
      } else {
 ...
 */
QueueWorker.prototype._reject = function(error) {
  var self = this,
      retries = 0;
  if (_.isNull(self.currentItemRef)) {
    self.busy = false;
    ...
    self.currentItemRef.transaction(function(queueItem) {
      ...
      if (error) {
        if (++retries < MAX_TRANSACTION_ATTEMPTS) {
          ...
          setImmediate(self._reject.bind(self), error);
        } else {
          logger.error(self._getLogEntry('reject item errored too many ' +
            'times, no longer retrying'), error);
        }
      } else {
        if (committed && existedBefore) {
 ...
 *   queue item.
 */
QueueWorker.prototype._tryToProcess = function(nextItemRef) {
  var self = this,
      retries = 0,
      malformed = false;
  if (!self.busy) {
    nextItemRef.transaction(function(queueItem) {
      ...
      if (error) {
        if (++retries < MAX_TRANSACTION_ATTEMPTS) {
          ...
          setImmediate(self._tryToProcess.bind(self), nextItemRef);
        } else {
          logger.error(self._getLogEntry('errored while attempting to claim ' +
            'a new item too many times, no longer retrying'), error);
        }
      } else if (committed && snapshot.exists()) {
        if (malformed) {
...
QueueWorker.prototype._isValidJobSpec = function(jobSpec) {
  if (_.isEmpty(jobSpec)) {
    return false;
  }
  if (!_.isNull(jobSpec.startState) &&
  ...
  if (!_.isString(jobSpec.finishedState)) {
    return false;
  }
  if (!_.isNull(jobSpec.jobTimeout) && (
