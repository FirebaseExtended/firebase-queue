 ...
 */
QueueWorker.prototype._reject = function() {
  var self = this,
      retries = 0,
      errorString = null,
      deferred = RSVP.defer();
...
QueueWorker.prototype._updateProgress = function(jobNumber) {
  ...
  var updateProgress = function(progress) {
    if (jobNumber !== self.jobNumber) {
      logger.warn(self._getLogEntry('No longer processing current item'));
      return RSVP.resolve();
    }
    ...
    if (_.isNull(self.currentItemRef)) {
      logger.warn(self._getLogEntry('No item currently being processed'));
      return RSVP.resolve();
    }
    return new RSVP.Promise(function(resolve, reject) {
      self.currentItemRef.transaction(function(queueItem) {
        ...
        if (committed && snapshot.exists()) {
          ...
          logger.warn(self._getLogEntry('Current item no longer owned by this process'));
          return resolve();
        }
      }, false);
    });
