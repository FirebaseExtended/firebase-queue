 ...
 */
QueueWorker.prototype._info = function(message) {
  var self = this;
  ...
  if (self.currentItemRef === null) {
    ...
    self.currentItemRef.transaction(function(queueItem) {
      ...
      if (committed && existedBefore) {
        self._info('completed ' + snapshot.key());
      }
      self.busy = false;
      self._tryToProcess(self.nextItemRef);
    }, false);
...
QueueWorker.prototype._reject = function(error) {
  ...
  if (self.currentItemRef === null) {
    ...
    self.currentItemRef.transaction(function(queueItem) {
      ...
      if (committed && existedBefore) {
        self._info('errored while attempting to complete ' + snapshot.key());
      }
      self.busy = false;
      self._tryToProcess(self.nextItemRef);
    }, false);
    ...
    nextItemRef.transaction(function(queueItem) {
      ...
      if (committed && snapshot.exists()) {
        if (self.busy) {
          ...
          self.busy = true;
          self._info('claimed ' + snapshot.key());
