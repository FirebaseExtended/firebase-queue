 ...
 */
QueueWorker.prototype._updateProgress = function(itemRef) {
  var self = this;

  var updateProgress = function(progress) {
    if (itemRef !== self.currentItemRef) {
      return RSVP.reject('No longer processing current item');
    }
    ...
    if (_.isNull(self.currentItemRef)) {
      return RSVP.reject('No item currently being processed');
    }
        ...
        if (malformed) {
          ...
          if (self.busy) {
            ...
            setImmediate(
              ...
              snapshot.val(),
              self._updateProgress(self.currentItemRef),
              self._resolve(),
              self._reject()
            );
          }
