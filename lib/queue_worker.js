...
function QueueWorker(authenticatedRef, processId, processingFunction) {
  ...
  self.queueRef = authenticatedRef;
...
QueueWorker.prototype.resolve = function(newQueueItem) {
  ...
  if (self.currentItemRef === null) {
    ...
    self.currentItemRef.transaction(function(queueItem) {
      ...
      if (currentState === self.inProgressState &&
          queueItem['_owner'] === self.uuid) {
        transactionResult = newQueueItem;
        if (typeof(transactionResult) !== 'object') {
          transactionResult = queueItem;
        }
        transactionResult['_state'] = self.finishedState;
        transactionResult['_state_changed'] = Firebase.ServerValue.TIMESTAMP;
        transactionResult['_owner'] = null;
        transactionResult['_progress'] = 100;
        transactionResult['_error_details'] = null;
