 ...
 * @return {Object}
 */
function QueueWorker(queueRef, processId, sanitize, processingFunction) {
  var self = this,
      error;
  ...
  var resolve = function(newQueueItem) {
    ...
    if ((jobNumber !== self.jobNumber) || _.isNull(self.currentItemRef)) {
      ...
      self.currentItemRef.transaction(function(queueItem) {
        ...
        if (queueItem._state === self.inProgressState &&
            queueItem._owner === self.processId + ':' + self.jobNumber) {
  ...
  if (!self.busy) {
    nextItemRef.transaction(function(queueItem) {
      ...
      if (queueItem._state === self.startState) {
        ...
        queueItem._state_changed = Firebase.ServerValue.TIMESTAMP;
        queueItem._owner = self.processId + ':' + (self.jobNumber + 1);
        ...
        if (malformed) {
          ...
          if (self.busy) {
            ...
            self.currentItemListener = self.currentItemRef
              ...
              /* istanbul ignore else */
              if (snapshot.val() !== self.processId + ':' + self.jobNumber &&
...
QueueWorker.prototype.setJob = function(jobSpec) {
  ...
  // Increment the jobNumber so that a job being processed before the change
  ...
  if (self._isValidJobSpec(jobSpec)) {
    ...
    self.newItemListener = self.newItemRef.on(
      'child_added',
      function(snapshot) {
    ...
    logger.error(self._getLogEntry('invalid job spec, not listening for new ' +
      'items'));
