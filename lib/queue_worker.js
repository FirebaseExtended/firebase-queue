      ...
      self.currentItemRef.transaction(function(queueItem) {
        ...
        if (error) {
          ...
          if (committed && existedBefore) {
            ...
            logger.warn(self._getLogEntry('Can\'t resolve item - current item no longer owned ' +
              'by this process'));
          }
          deferred.resolve();
          self.busy = false;
      ...
      self.currentItemRef.transaction(function(queueItem) {
        ...
        if (error) {
          ...
          if (committed && existedBefore) {
            ...
            logger.warn(self._getLogEntry('Can\'t reject item - current item no longer owned ' +
              'by this process'));
          }
          deferred.resolve();
          self.busy = false;
...
QueueWorker.prototype._updateProgress = function(jobNumber) {
  ...
  var updateProgress = function(progress) {
    if (jobNumber !== self.jobNumber) {
      logger.warn(self._getLogEntry('Can\'t update progress - no longer processing current item'));
      return RSVP.resolve();
    }
    ...
    if (_.isNull(self.currentItemRef)) {
      logger.warn(self._getLogEntry('Can\'t update progress - no item currently being processed'));
      return RSVP.resolve();
    }
    return new RSVP.Promise(function(resolve, reject) {
      self.currentItemRef.transaction(function(queueItem) {
        ...
        /* istanbul ignore if */
        if (error) {
          var errorMsg = 'errored while attempting to update progress';
          logger.error(self._getLogEntry(errorMsg), error);
        ...
        if (committed && snapshot.exists()) {
          ...
          logger.warn(self._getLogEntry('Can\'t update progress - current item no longer owned by' +
            ' this process'));
          return resolve();
        }
      }, false);
