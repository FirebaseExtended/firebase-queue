...
QueueWorker.prototype.resetJob = function(jobSpec) {
  ...
  if (jobSpec.timeout !== null && (
      typeof(jobSpec.timeout) !== 'number' ||
      jobSpec.timeout % 1 !== 0 ||
      jobSpec.timeout <= 0)) {
  ...
  self.newItemRef = self.ref.orderByChild('_state').equalTo(self.startState);
...
QueueWorker.prototype.setUpTimeouts = function() {
  ...
  if (self.jobTimeout) {
    self.processingItemsRef = self.ref.orderByChild('_state')
